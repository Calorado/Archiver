...
// Note: The last call to aes_xtime() is unneeded, but often ends up generating a smaller binary
...
static uint8_t Multiply(uint8_t x, uint8_t y)
{
  return (((y & 1) * x) ^
       ...
       ((y>>4 & 1) * aes_xtime(aes_xtime(aes_xtime(aes_xtime(x)))))); /* this last call to aes_xtime() can be omitted */
  }
#else
#define Multiply(x, y)                                \
      (  ((y & 1) * x) ^                              \
